...
leftOrRight <- function(eyeData, verteporfin) {
  whoIsVert <- subset(verteporfin, select = c(PERSON_ID))
  routeID <- c(left, right, bi)
  memoData <- eyeData %>%
    filter(ROUTE_CONCEPT_ID %in% routeID) %>%
    select(PERSON_ID, ROUTE_CLASS) %>%
  ...
  memoData <- memoData%>%
    group_by(PERSON_ID) %>%
    arrange(ROUTE_CLASS) %>%
    mutate(ALL_ROUTE = paste(ROUTE_CLASS, collapse = "-")) %>%
    select(PERSON_ID, ALL_ROUTE) %>% distinct()
  
  # 양안의심 
  doupt1 <- memoData %>%
    filter(grepl('Bi', ALL_ROUTE)) 
  doupt2 <- memoData %>%
    filter(grepl('Left-Right', ALL_ROUTE)) 
  
  # 양안제외
  leftP <- memoData %>%
    filter(! PERSON_ID %in% doupt1$PERSON_ID) %>%
    filter(! PERSON_ID %in% doupt2$PERSON_ID) %>%
    filter(ALL_ROUTE == "Left") %>%
    filter(PERSON_ID %in% whoIsVert$PERSON_ID)
  
  rightP <- memoData %>%
    filter(! PERSON_ID %in% doupt1$PERSON_ID) %>%
    filter(! PERSON_ID %in% doupt2$PERSON_ID) %>%
    filter(ALL_ROUTE == "Right") %>%
    filter(PERSON_ID %in% whoIsVert$PERSON_ID)
  
  ## 원래 data 수정
  verteporfin <- verteporfin %>%
    mutate(ROUTE_CLASS = replace(ROUTE_CLASS, PERSON_ID %in% leftP$PERSON_ID, "Left"))
  verteporfin <- verteporfin %>%
    mutate(ROUTE_CLASS = replace(ROUTE_CLASS, PERSON_ID %in% rightP$PERSON_ID, right)) 
  
  verteporfin <-verteporfin %>%
    filter(ROUTE_CLASS =="Left" | ROUTE_CLASS =="Right")
  return(
    verteporfin
  )
}
